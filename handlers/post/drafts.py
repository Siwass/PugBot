...
from services.channel_access import resolve_publish_chat_id
...
async def draft_publish(callback: CallbackQuery, state: FSMContext):
    ...
    async with AsyncSessionLocal() as session:
        ...
        # Канал только из числа каналов автора (ownership)
        chat_id = await resolve_publish_chat_id(
            callback.from_user.id,
            channel_id=post.channel_id,
        )
        if chat_id is not None and post.channel_id is None:
